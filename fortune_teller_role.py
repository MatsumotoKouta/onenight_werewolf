...
async def draw_fortune_teller_role_scene(players, fortune_tell_index=-1):
    ...
    if fortune_tell_index >= 0:
        ...
        text = (
            f"墓地に残された2つの役職は「{players[len(players) - 2]['role']}」と「{players[len(players) - 1]['role']}」です。"
            )
...
async def update_fortune_teller_role_scene(player_index):
    ...
    if BUTTON.get_rect().collidepoint(pos):
        player_index += 1
        return SCENE_PLAYER, player_index
    else:
        return SCENE_FORTUNE_TELLER_ROLE, player_index
