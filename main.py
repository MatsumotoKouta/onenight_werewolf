    ...
    while True:
        ...
        if scene == SCENE_TITLE:
            ...
            if player_index < len(players) - 2:
                ...
                villager_poll = sorted(players[:-2], key=lambda p: int(p[
                ...
                for i in range(len(villager_poll)):
                    if i == 0 or villager_poll[i - 1]['villager_poll_count'
                        ...
                        villager_poll_result.append(villager_poll[i])
                    else:
                        break
                if len(villager_poll_result) == len(villager_poll):
                    most_suspicious_player = players[random.randrange(len(
                        players) - 2)]
                most_suspicious_player = villager_poll_result[0]
                scene = SCENE_DISCUSS
